class PageGraph():
    def serveGraphs(self, qs):
        ...
        self.end_headers()
        if self.getLevel() > 1:
            buttons = "<button onclick='button(\" + i + \", this)'>Pause</button><a href='/graph?edit=\" + i + \"'><button>Edit graphs</button></a>"
        else:
            buttons = "<button onclick='button(\" + i + \", this)'>Pause</button>"
        self.serveHeader("graph", qs, True, True, {"BUTTONS": buttons})
        self.serveBody("graph", qs)
        self.serveFooter()
    ...
    def serveGraphsForm(self, qs):
        ...
        while i < graphlen:
            ...
            i += 1
        i = 0
        settings = self.settings.getCfgValue("graphMaxDisplay")
        if isinstance(settings, str):
            settings = [settings]
        while i < graphlen:
            tgraphForm[i] += settings[i] + ";"
    ...
    def serveAddGraph(self, qs, info = ''):
        self.send_response(200)
        self.sendDefaultHeaders()
        self.end_headers()
    ...
    def serveGraphSettings(self, qs, isSession = False):
        if not isSession:
            self.setSession(qs)
